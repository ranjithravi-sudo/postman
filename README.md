...
5. **Bearer token (optional):** To use a stored API token instead of pasting it in the UI each time, copy `bearer-token.example.txt` to `bearer-token.txt`, then paste your Bearer token in `bearer-token.txt` (one line). The app will use it for all runs unless you override it in the "Bearer token (optional)" field in the UI.
